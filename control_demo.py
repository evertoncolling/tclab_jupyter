...
class GUI(object):
    ...
    def __init__(self):
        ...
        ##############################################################################
        # Define colors: T1, T1_SP, T2, T2_SP, _Q1, _Q2
        colors = ['#1f77b4','#5ebdff','#d62728','#ff7f0e','#1f77b4','#d62728']
        ...
        fig1 = bq.Figure(layout=fig_lay_up, axes=[ax_x1, ax_T1], 
                         marks=[self._T1_meas,self._T1_set_point], fig_margin=box1)

        fig2 = bq.Figure(layout=fig_lay_up, axes=[ax_x1, ax_T2], 
                         marks=[self._T2_meas,self._T2_set_point], fig_margin=box2)
        ...
        self._PT1 = wi.FloatProgress(value=self._Tc0[0]-273.15, min=0, max=100.0,
        ...
        self._PT2 = wi.FloatProgress(value=self._Tc0[1]-273.15, min=0, max=100.0,
                               description='<b>PV (°C):</b>',
                               bar_style='warning',
                               orientation='horizontal',
                               style=style)
        ...
        self._tQ1 = wi.BoundedFloatText(value=self._Q10,min=0,max=100.0,step=0.5,
                                  layout=wi.Layout(width='60px'))
        wi.jslink((self._wQ1, 'value'), (self._tQ1, 'value'))

        self._bQ1 = wi.Button(description='Set',layout=wi.Layout(width='50px'))
        ...
        self._wQ2 = wi.FloatSlider(value=self._Q20,min=0,max=100.0,step=0.5,
                             ...
                             readout=False,style=style,
                             layout=wi.Layout(width='230px'))
        self._tQ2 = wi.BoundedFloatText(value=self._Q20,min=0,max=100.0,step=0.5,
                                  layout=wi.Layout(width='60px'))
        wi.jslink((self._wQ2, 'value'), (self._tQ2, 'value'))

        self._bQ2 = wi.Button(description='Set',layout=wi.Layout(width='50px'))
        self._bQ2.on_click(self._Q2_click)

        self._wT1 = wi.FloatSlider(value=self._T1_SP,min=20,max=60.0,step=0.5,
                             description='<b>T1 SP:</b>',
                             continuous_update=False,orientation='horizontal',
                             readout=False,style=style,
                             disabled=True,
                             layout=wi.Layout(width='230px'))
        self._tT1 = wi.BoundedFloatText(value=self._T1_SP,min=20,max=60.0,step=0.5,
                                  disabled=True,
                                  layout=wi.Layout(width='60px'))
        wi.jslink((self._wT1, 'value'), (self._tT1, 'value'))

        self._bT1 = wi.Button(description='Set',layout=wi.Layout(width='50px'),disabled=True)
        self._bT1.on_click(self._T1_click)

        self._wT2 = wi.FloatSlider(value=self._T2_SP,min=20,max=60.0,step=0.5,
                             description='<b>T2 SP:</b>',
                             continuous_update=False,orientation='horizontal',
                             disabled=True,
                             readout=False,style=style,
                             layout=wi.Layout(width='230px'))
        self._tT2 = wi.BoundedFloatText(value=self._T2_SP,min=20,max=60.0,step=0.5,
                                  disabled=True,
                                  layout=wi.Layout(width='60px'))
        wi.jslink((self._wT2, 'value'), (self._tT2, 'value'))

        self._bT2 = wi.Button(description='Set',layout=wi.Layout(width='50px'),disabled=True)
        self._bT2.on_click(self._T2_click)
        
        ##############################################################################
        #                                                              _MODE SELECTION
        ##############################################################################
        self._mode = wi.ToggleButtons(options=['Manual', 'On-Off', 'PID', 'MPC'],
        ...
        #                                                                 STOP THREAD
        ##############################################################################
        self._b_stop = wi.Button(description='Stop',button_style='warning',icon='stop',
        ...
        ##############################################################################
        self._b_play = wi.Button(description='Start',button_style='success',icon='play',
                                layout=wi.Layout(width='100px',height='32px'))
        ...
        Q1_Set = wi.HBox((h_space,self._wQ1,self._tQ1,self._bQ1),
                         layout=wi.Layout(border='solid 2px gray',width='360px'))
        ...
        figy = wi.VBox((fig1x,v_space,fig2x),
                        layout=wi.Layout(border='solid 2px gray',width='800px'))
        ...
        self._conf11 = wi.HBox((
            wi.HTML(value='<p style="text-align: right;"><b>&Delta;t (s):</b></p>',
                    layout=lay),
            wi.FloatSlider(value=4.0,min=1.0,max=10.0,step=0.5,
                           description='',style=style)))
        
        self._conf12 = wi.HBox((
            wi.HTML(value='<p style="text-align: right;"><b>Sleep time (s):</b></p>',
                    layout=lay),
        ...
        self._conf21 = wi.HBox((
            wi.HTML(value='<p style="text-align: right;"><b>Deadband (K):</b></p>',
                    layout=lay5),
        ...
        self._conf31 = wi.HBox((
            wi.HTML(value='<p style="text-align: right;"><b>Kc (K/%Heater):</b></p>',
                    layout=lay4),
        ...
        conf413 = wi.HBox((but41,but42),layout=wi.Layout(margin='10px 0 0 0'))

        ##############################################################################
        #                                                         CONFIGURATOR LAYOUT
        ##############################################################################
        tab = wi.Tab([wi.VBox((self._conf11,self._conf12,
                               wi.Label(layout=wi.Layout(height='206px')),conf13)), 
                      wi.VBox((wi.HBox((box21,box22)),
                               wi.Label(layout=wi.Layout(height='191px')),conf24)),
                      wi.VBox((wi.HBox((box31,box32)),
                               wi.Label(layout=wi.Layout(height='127px')),conf38)),
    ...
    def _MPC(self):
        ...
        m.Q_C12 = m.Intermediate(m.U*m.As*(m.T2i-m.T1i)) # Convective
        m.Q_R12 = m.Intermediate(m.eps*m.sigma*m.As*(m.T2i**4-m.T1i**4)) # Radiative

        # Semi-fundamental correlations (energy balances)
        m.Equation(m.TH1.dt() == (1.0/(m.mass*m.Cp))*(m.U*m.A*(m.Ta-m.T1i) \
                            + m.eps * m.sigma * m.A * (m.Ta**4 - m.T1i**4) \
                            + m.Q_C12 + m.Q_R12 \
                            + m.alpha1*m.Q1))

        m.Equation(m.TH2.dt() == (1.0/(m.mass*m.Cp))*(m.U*m.A*(m.Ta-m.T2i) \
                            + m.eps * m.sigma * m.A * (m.Ta**4 - m.T2i**4) \
                            - m.Q_C12 - m.Q_R12 \
                            + m.alpha2*m.Q2))
    ...
    def _work_pid(self):
        ...
        while self._flag:
            ...
            [Q10,ierr1] = self._PID(self._T1_SP,T[-1,0]-273.15,T[-2,0]-273.15,ierr1,self._delta_t,
                                   self._pid1_gain,self._pid1_reset,self._pid1_rate)
            [Q20,ierr2] = self._PID(self._T2_SP,T[-1,1]-273.15,T[-2,1]-273.15,ierr2,self._delta_t,
                                   self._pid2_gain,self._pid2_reset,self._pid2_rate)
